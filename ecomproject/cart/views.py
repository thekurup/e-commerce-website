...
def view_cart(request):
    if request.session.has_key('userlogin'):
        ...
        items = CartItems.objects.filter(user=id)
        cart_sub_total = 0
        for item in items:
        ...
        id = _session_id(request)
        cart_item = CartItems.objects.filter(anonymous_user=id).order_by('id')
        categories = Category.objects.all()
        subcategories = SubCategory.objects.all()
        product = Products.objects.all()
        items = CartItems.objects.filter(anonymous_user=id)
        cart_sub_total = 0
        for item in items:
            cart_sub_total = cart_sub_total + item.sub_total
        context = {
...
def add_cart(request):
    if request.user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            if CartItems.objects.filter(products_id=single_product, user=request.user).first():
                ...
                Wishlist.objects.filter(products_id=single_product).delete()
                return JsonResponse(context, safe=False)
        ...
        if request.method == 'POST':
            single_product = Products.objects.get(id=product_id)
            if CartItems.objects.filter(products_id=single_product, anonymous_user=guest_user).first():
                return JsonResponse('', safe=False)

            else:
                product_sub_total = single_product.sale_price * 1
                cart = CartItems(anonymous_user=guest_user, products_id=single_product, quantity=1,
...
def product_increment(request):
    if request.user.is_authenticated:
        ...
        if cart_item.quantity < product.stocks:
            ...
            cart_item = CartItems.objects.filter(user=user_id, products_id=prd_id)
        ...
        user_cart = CartItems.objects.filter(user=user_id)
        ...
        guest_user_id = _session_id(request)
        prd_id = request.POST['prd_id']
        ...
        if cart_item.quantity < product.stocks:
            ...
            cart_item = CartItems.objects.filter(anonymous_user=guest_user_id, products_id=prd_id)
        try:
            for i in cart_item:
                price = i.products_id.sale_price
                qnty = i.quantity
        ...
        user_cart = CartItems.objects.filter(anonymous_user=guest_user_id)
...
def product_decrement(request):
    if request.user.is_authenticated:
        ...
        if cart_item.quantity > 1:
            ...
            print(cart_item.quantity)
            cart_item.save()
            cart_item = CartItems.objects.filter(anonymous_user=guest_user, products_id=prd_id)
            for i in cart_item:
                price = i.products_id.sale_price
                qnty = i.quantity
                sub_total = (price * qnty)
            ...
            user_cart = CartItems.objects.filter(anonymous_user=guest_user)
            cart_sub_total = 0
            for item in user_cart:
                cart_sub_total = cart_sub_total + item.sub_total
...
def checkout(request):
    if request.user.is_authenticated:
        ...
        try:
            ...
            cart_items = CartItems.objects.filter(user=user_cart)
            sub_total_total = CartItems.objects.filter(user=user_cart)
            sum = 0
...
def add_address(request):
    ...
    new_address = UserAddress.objects.create(first_name=first_name, last_name=last_name, email=email,
                                             phone_number=phone_number, address=address, city=city, post_code=post_code,
                                             country=country, state=state, user=cart_user)
...
def place_order(request):
    if request.method == 'POST':
        address_id = request.POST['mailing_address']
         ...
         #wallet if any
        if request.POST['hidden_g_total']:
            g_total =request.POST['hidden_g_total']
            g_total = int(g_total)
            user = request.user.id
            user= Accounts.objects.get(id=user)
            user.wallet_amount = user.wallet_amount - g_total
            user.save()
        ...
        if request.POST['grand_total']:
            ...
            cart_item = CartItems.objects.filter(user=request.user.id)
            grand_total = 0
        ...
        order = Order.objects.create(order_id=order_id, payment_method=payment_method, user=ordered_user,
                                     delivered_address=ordered_address, delivery_status=order_status,
                                     grand_total=grand_total, payment_status=payment_status)
        ...
        for items in cart_items:
            ...
            OrderItems.objects.create(order=single_order, user=user, products_id=product_id, quantity=quantity,
                                      sub_total=sub_total, is_active=is_active)
        ...
        Payment.objects.create(user=ordered_user, payment_method=payment_method, payment_id=payment_id,
                               amount_paid=grand_total, status=order_status, order=order)
        ...
        if request.POST['apply_coupon']:
            coupon = request.POST['apply_coupon']
            coupon_instance = Coupon.objects.get(coupon_id=coupon)
            user = request.user
...
def paypal_payment(request):
    ...
    new_order = Order.objects.create(user=current_user, order_id=body['orderID'],
                                     payment_method=body['payment_methods'],
                                     delivery_status=delivery_status, payment_status=body['status'],delivered_address = ordered_address,
                                     grand_total=body['grand_total'] )

    Payment.objects.create(user=current_user, payment_id=body['transID'], payment_method=body['payment_methods'],
                           amount_paid=body['grand_total'], status=body['status'], order=new_order)
...
def coupon_apply(request):
    ...
    if ExpiredCoupon.objects.filter(coupon=coupon, user=user).exists():
        ...
        cart_items = CartItems.objects.filter(user=request.user.id)
