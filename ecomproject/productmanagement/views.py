...
def add_products(request):
    if request.method == 'POST':
        ...
        format, img1 = cover.split(';base64,')
        ext = format.split('/')[-1]
        product_cover = ContentFile(base64.b64decode(img1), name= product_name + '1.' + ext)
...
def edit_product(request, id):
    if request.method == 'POST':
        ...
        new_pro_image1 = request.FILES.get('product_cover1')
        new_pro_image2 = request.FILES.get('product_cover2')
        new_pro_image3 = request.FILES.get('product_cover3')
        new_pro_image4 = request.FILES.get('product_cover4')

        # updating first image

        if request.FILES.get('product_cover1'):
            new_pro_image1 = request.FILES.get('product_cover1')
            single_image1 = ImageGallery.objects.get(id = pro_image_id1)
            single_image1.image = new_pro_image1
            single_image1.save()

        # updating second image


        if request.FILES.get('product_cover2'):
            new_pro_image2 = request.FILES.get('product_cover2')
            single_image2 = ImageGallery.objects.get(id = pro_image_id2)
            single_image2.image = new_pro_image2
            single_image2.save()

        # updating third image


        if request.FILES.get('product_cover3'):
            new_pro_image3 = request.FILES.get('product_cover3')
            single_image3 = ImageGallery.objects.get(id = pro_image_id3)
            single_image3.image = new_pro_image3
            single_image3.save()

        # updating fourth image


        if request.FILES.get('product_cover4'):
            new_pro_image4 = request.FILES.get('product_cover4')
            single_image4 = ImageGallery.objects.get(id = pro_image_id4)
            single_image4.image = new_pro_image4
            single_image4.save()
...
def search(request):
    if 'search_keyword' in request.GET:
        ...
        if search_keyword:
            products= Products.objects.order_by('-product_name').filter(Q(product_name__icontains=search_keyword) | Q(description__icontains=search_keyword))

            return render(request, 'user/product_grid_view.html', {'products':products})
